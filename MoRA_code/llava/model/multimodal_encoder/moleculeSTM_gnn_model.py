...
class GNN_graphpred(nn.Module):
    def __init__(
        ...
        self.is_loaded = False  # 添加is_loaded属性
        ...
        if init_checkpoint is not None:
            ...
            self.is_loaded = True  # 如果加载了checkpoint，设置为True
    ...
    def encode_mol(self, mol, proj=False, return_node_feats=False, eval=True):
        ...
        mol = mol.to(device)
        if eval:
            self.molecule_node_model.eval()
            with torch.no_grad():
                h_graph, h_node = self.forward(mol)
        else:
            self.molecule_node_model.train()
            h_graph, h_node = self.forward(mol)
        ...
        for key, value in state_dict.items():
            if key in model_state_dict:
                if value.shape == model_state_dict[key].shape:
                    ...
                    print(f"Skipping {key} due to shape mismatch: {value.shape} vs {model_state_dict[key].shape}")
            else:
                print(f"Skipping {key} as it's not in current model")
