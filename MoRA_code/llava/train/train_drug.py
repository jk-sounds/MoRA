...
def print_model_structure(model):
    def recursive_print(module, prefix='', depth=0):
        ...
        print(f"{indent}[{type(module).__name__}] {prefix}".ljust(60), end='',flush=True)
        own_params = [name for name, _ in module.named_parameters(recurse=False)]
        print(f"Params: {own_params if own_params else 'None'}",flush=True)
        for name, child in module.named_children():
            child_prefix = f"{prefix}.{name}" if prefix else name
            recursive_print(child, child_prefix, depth + 1)
    print("="*50 + " Model Structure " + "="*50,flush=True)
    recursive_print(model)
    print("="*50 + " Parameter Key Examples " + "="*50,flush=True)
    for i, (name, param) in enumerate(model.named_parameters()):
        print(f"[{i+1:03d}] {name}",flush=True)
...
class ModelArguments:
    ...
    mm_projector_type: Optional[str] = field(default='mora')
    mm_use_im_start_end: bool = field(default=False)
    mm_use_im_patch_token: bool = field(default=True)
    pretrain_prefusion: Optional[str] = field(default=None)
    prefusion_layer_num: Optional[int] = field(default=None)
    get_fusion: bool = field(default=False)
    # lora generater args
    mora_dim: Optional[int] = field(default=1024)  
    mora_depth: Optional[int] = field(default=4)
    mora_visual_dim: Optional[int] = field(default=300) 
    mora_pos_num: Optional[int] = field(default=21)
    mora_llm_dim: Optional[int] = field(default=4096)
    mora_llm_depth: Optional[int] = field(default=32)
    mora_rank: Optional[int] = field(default=8)
    mora_type: Optional[str] = field(default='qkvom')
    mora_alpha: Optional[int] = field(default=None)
...
def get_mm_adapter_state_maybe_zero_3(named_params, keys_to_match):
    to_return = {k: t for k, t in named_params if any(key_match in k in keys_to_match)}
    to_return = {k: maybe_zero_3(v, ignore_status=True).cpu() for k, v in to_return.items()}
    return to_return
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        keys_to_match = ['mm_projector', 'layer_attention_transform', 'layer_attention_scorer', 'fpn_head_module']
...
def train():
    ...
    if model_args.graph_tower is not None:
        ...
        elif model_args.mm_projector_type == 'mora':
            model = LlavaLlamaMoRAForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                **bnb_model_from_pretrained_args
            )
