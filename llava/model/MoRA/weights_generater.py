...
class LoRAGenerater(nn.Module):
    def __init__(self, dim, depth, \
                 ...
                 mlp_ratio=4, num_heads=16, skip_layers=1, mora_alpha=None):
        super().__init__()
        ...
        self.mora_alpha = mora_alpha if mora_alpha is not None else lora_rank
    ...
    def forward(self, img_feature):
        ...
        img_feature = self.visual_proj(img_feature) 
        if self.weights_sep:
            weights = []
        ...
        Bs = self.mora_alpha / self.lora_rank * Bs
        lora_weights_list = []
...
def get_lora_generater(model_args):
    return LoRAGenerater(
            dim = model_args.mora_dim,
            depth = model_args.mora_depth,
            visual_dim = model_args.mora_visual_dim,
            pos_num = model_args.mora_pos_num,
            llm_dim = model_args.mora_llm_dim,
            llm_depth = model_args.mora_llm_depth,
            lora_rank = model_args.mora_rank,
            lora_type = model_args.mora_type,
            weights_sep = model_args.weights_sep,
            skip_layers = model_args.skip_layers,
            mora_alpha = model_args.mora_alpha,
    )
