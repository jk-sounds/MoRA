...
def evaluate(input_file, morgan_r, verbose=False):
    ...
    for i, (desc, gt_m, ot_m) in enumerate(enum_list):
        ...
        if i % 100 == 0:
            if verbose: print(i, 'processed.',flush=True)
    ...
    if verbose:
        print('Average MACCS Similarity:', maccs_sims_score, flush=True)
        print('Average RDK Similarity:', rdk_sims_score, flush=True)
        print('Average Morgan Similarity:', morgan_sims_score, flush=True)
