    ...
    with open(args.in_file, "rb") as f:
        ...
        for i, raw in enumerate(list_data_dict):
            if args.task == "retrosynthesis":
                ...
                batch.append((instruction, graph, raw['output']))
            elif args.task == "molcap":
                graph = smiles2graph(selfies2smiles(raw['input']))
                if args.add_selfies:
                    instruction = raw['instruction'] + f" The molecule is: {raw['input']}"
                else:
                    instruction = raw['instruction']
                batch.append((instruction, graph, raw['output']))
            else:
...
def main(args):
    ...
    # 在第135行左右，load_pretrained_model调用之后添加
    ...
    for instructions, graphs, gts in tqdm(
        ...
        # elif '\n' in output:
        #     output = output.split('\n')[0]
        # output = tokenizer.batch_decode(output_ids, skip_special_tokens=False, spaces_between_special_tokens=False)[0].strip().replace('</s>', '')
